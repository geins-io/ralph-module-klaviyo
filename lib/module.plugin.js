...
export default function (app, inject) {
  ...
  const subscribeToList = async (user) => {
    const url = `${options.listBaseUrl}/${options.listId}/subscribe?api_key=${options.apiKey}`
    const fetchOptions = {
      method: 'POST',
      headers: { 
    ...
    try {
      ...
      console.log('vidar data', data);
    } catch (error) {
      console.error('vidar error: ', error);
    }
  }
